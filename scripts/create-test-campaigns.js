...
async function run() {
    ...
    for (const persona of personas) {
        try {
            ...
            const adGroupId = await createAdGroup(campaignId, `${persona.name} — Main`, 2_000_000);
            console.log(`  ✅ Campaign: ${campaignId} | Ad Group: ${adGroupId}\n`);
        }
        catch (e) {
            console.error(`  ❌ Failed for ${persona.slug}:`, e.message);
            if (e.errors)
                e.errors.forEach((err) => console.error('   ', err.message, JSON.stringify(err.location?.field_path_elements)));
        }
    }
    await pool.end();
    console.log('Done.');
...
//# sourceMappingURL=create-test-campaigns.js.map