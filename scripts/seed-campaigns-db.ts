...
for (const c of campaigns) {
  const persona = await queryOne<{ id: number }>('SELECT id FROM personas WHERE slug = $1', [c.slug]);
  if (!persona) { console.error('Persona not found:', c.slug); continue; }
  await query(
    `INSERT INTO campaigns (persona_id, google_campaign_id, name, status)
     VALUES ($1, $2, $3, 'paused')
     ON CONFLICT (google_campaign_id) DO NOTHING`,
    [persona.id, c.googleId, c.name],
  );
