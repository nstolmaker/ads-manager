...
function score(s:string){const t=s.toLowerCase();for(const p of ignorePatterns){ if(t.includes(p)) return 'ignore'; }for(const p of intentPatterns){ if(t.includes(p)) return 'intent'; }if(/for (everyone|beginners|all|humans|good)/.test(t)) return 'ignore';return 'maybe';}

const intents = new Set<string>();
for (const seed of ['ai','llm','chatgpt','claude']) for (const q of json[seed] || []) if (score(q)==='intent') intents.add(q.toLowerCase());
const kws=[...intents];
...
for(const r of rows.filter(r=>r.monthlySearches>0).slice(0,30)) console.log(`${String(r.monthlySearches).padStart(5)}/mo | ${String(r.competition).padEnd(6)} | idx ${String(r.competitionIndex).padStart(3)} | $${r.lowTopOfPage}-$${r.highTopOfPage} | ${r.keyword}`);
